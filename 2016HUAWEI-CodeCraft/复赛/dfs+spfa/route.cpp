...
using namespace std;
	...
	int 	conflict_val=10000000;
	int 	second_conflict_val = 10000000;
...
bool jumpOut = 0;
	...
	{
		conflict_val=second_conflict_val;
...
*/
			...
			if(ans_one==INF||ans_two==INF)////////////如果拢总跑不出来，那就退回去吧
				goto four;
			if(repeat_num==0)
				goto ans_end;
			init_path_one();
...
four:		if(ans_one == INF || ans_two == INF)
		{
			if(is_reverse)
			{
			...
			{
				result1[0] = 0;
				record_result(WORK_PATH,result1[0]);
			}
			else{
				for(int i=0;i<(int)final_result1.size();i++)
				{
			...
			if(ans_two == INF)
			{
				result2[0] = 1;
				return;
				//record_result(BACK_PATH,result2[0]);
			}
			else{
				for(int i=0;i<(int)final_result2.size();i++)
				{
		...
		four:	if (is_data15)
			{
				...
				puts("");
				if(final_result1.size()==0||final_result2.size()==0)
				{
					goto end11;
				}
